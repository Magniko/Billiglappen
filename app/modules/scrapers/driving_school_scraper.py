...
def Main():
    ...
    if len(sys.argv) <= 1:
        ...
        place_ids = (
            scrape(viken_locations_rural, 25000)
            + scrape(viken_locations_urban, 10000)
            + scrape(viken_locations_metro, 2500)
            + scrape(oslo, 500)
        )
    ...
    for place_id in place_ids:
        ...
        if (
            data["address_components"][-1]["long_name"] not in ignore_codes
            and "website" in data
        ):

            # initializing row to be added to dataframe
            row = {
                "id": get_id(data["name"], data["address_components"][-4]["long_name"]),
                "name": data["name"],
                "website": data["website"]
                .replace("www.", "")
                .replace("https://", "")
                .replace("http://", ""),
                "address": data["formatted_address"].replace(", Norway", ""),
                "municipalty": data["address_components"][-4]["long_name"],
    ...
    for location in locations:
        ...
        place_ids.extend(
            [
                ...
                if i["business_status"] != "CLOSED_PERMANENTLY"
            ]
        )
...
def get_details(place_id):
    ...
    params = urllib.parse.urlencode(
        {
            "place_id": place_id,
            "fields": "name,website,formatted_address,address_components,geometry,formatted_phone_number,rating",
            "key": API_KEY,
        },
        safe=",",
    )
...
def get_id(school, municipality):
    ...
    id_number = str(
        int(hashlib.sha256(" ".join(id_string).encode("utf-8")).hexdigest(), 16)
    )[-4:]
